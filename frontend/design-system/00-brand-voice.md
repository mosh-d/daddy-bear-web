...
Home does use a countdown (`components/Countdown.tsx`) — an earlier version of this document ruled that out entirely as an urgency device borrowed from Black Market's tone. The distinction that changed it: Black Market's countdown counts down to a manufactured event (a record attempt built specifically to need a clock); ours counts down to something that's true regardless of whether we display it. Until 15 August 2026 that was principal photography starting; it now counts down to the next ticketed screening, next to that screening's card. That's information, not hype, so it passes the working test above: it would still make sense read aloud at a family screening ("the Abuja screening is in six days").
